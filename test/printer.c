...

{
	...
	struct formats f_types[] = {
		{'c', print_char}, {'s', print_str}, {'%', print_perc}, /*{'i', print_num},
		{'d', print_num}, {'b', print_bin}, {'u', print_uns}, {'o', print_oct},
		{'x', print_hex}, {'X', print_HEX}, {'p', print_poi}, {'S', print_cus},
		{'r', print_rev}, {'R', print_rot}, */{'\0', NULL}
	};

	for (j = 0; f_types[j].form != '\0'; j++)
		if (format[*i] == f_types[j].form)
			return (f_types[j].id(args, buff, flag, width, precis, size));
